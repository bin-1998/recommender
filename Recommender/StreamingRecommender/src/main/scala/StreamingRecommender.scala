...
object StreamingRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val simMovieMatrix=sparkSession.read
      ...
      .load()  //返回的是DataFrame 类型
      .as[MovieRecs]
      .rdd
      .map {  //小括号　写一行　　大括号写多行
        movieRecs => //为了查询方便　转换成键值对　ｍａｐ　形式
          (movieRecs.mid,movieRecs.recs.map(x => (x.mid, x.score)).toMap) //元组可以直接转键值对　　Recommender　样域类不可以转
      }.collectAsMap()   //toMap转化成键值对   集合  去重

    val simMovieMatrixBroadCast=sc.broadcast(simMovieMatrix)  //广播  //每个节点都知道
    ...
    // 通过kafka创建一个DStream  //kafka-》sparkStream
    val kafkaStream=KafkaUtils.createDirectStream[String,String](ssc,
      LocationStrategies.PreferConsistent,
      ConsumerStrategies.Subscribe[String,String](Array(config("kafka.topic")),kafkaParam)
    )//消费者策略  供给sparkStream

    //把原始数据UID|MID|SCORE|TIMESTAMP 传成评分流   传进来的就是文本字符String   每一行都是键值对
    val ratingStream=kafkaStream.map{    //就分割一下数据　变成元组
      msg=>
        val attr=msg.value().split("\\|")
        (attr(0).toInt,attr(1).toInt,attr(2).toDouble,attr(3).toInt)
    }

    //继续做流式处理,核心实时算法部分      kafkaStream->ratingStream->
    ratingStream.foreachRDD{
      rdds=>rdds.foreach{
        case (uid,mid,score,timestamp)=> {
          println("rating data coming! >>>>>>>>>>>>>>>")

          //1.从redis里获取当前用户最近的k次评分,保存成Array[()]
  ...
  def getUserRecentlyRating(num: Int, uid: Int, jedis: Jedis):Array[(Int,Double)]={
    //从redis读取数据,用户评分数据保存在　uid:UID为key的队列里,value是 MID:SCORE
  ...
  def getTopSimMovies(num: Int, mid: Int, uid: Int, simMovies: scala.collection.Map[Int, scala.collection.immutable.Map[Int, Double]])
    ...
    val ratingExist = ConnHelper.mongoClient(mongoConfig.db)(MONGODB_RATING_COLLECTION)
      ...
      .toArray
      .map {
        items => items.get("mid").toString.toInt
      }
  ...
  }//需要return 一个参数　,这里有两个所以报错

  def computeMovieScores(candidataMovies: Array[Int],
    ...
    for (candidataMovie <- candidataMovies; userRecentlyRating <- userRecentlyRatings) {
      //拿到备选电影和最近的评分电影的相似度
      val simScore = getMoviesSimScore(candidataMovie, userRecentlyRating._1, simMovies)

      if (simScore > 0.7) {
        //备选计算电影的基础得分
        scores += ((candidataMovie, simScore * userRecentlyRating._2))
        if (userRecentlyRating._2 > 3) {
          increMap(candidataMovie) = increMap.getOrDefault(candidataMovie, 0) + 1
        } else {
          decreMap(candidataMovie) = decreMap.getOrDefault(candidataMovie, 0) + 1
        }
      }
    }
    //根据备选电影的mid做groupby,根据公式去求　最后的推荐评分
    scores.groupBy(_._1).map {
      case (mid, scoreList) =>
        (mid, scoreList.map(x => x._2).sum / scoreList.length + log(increMap.getOrDefault(mid, 1)) - log(decreMap.getOrDefault(mid, 1)))
    }.toArray
  }
